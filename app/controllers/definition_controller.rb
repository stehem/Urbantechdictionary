...
def show
  ...
  word = defi.word
  render :json => {:word => { :word => word.word}, :definition => {:definition => defi.definition, :upv => defi.upv, :dwv => defi.dwv, :id => defi.id }}
end
...
def create
  word = Word.where('word = ?', params[:word]).first
    if word
      ...
      defi = newword.definitions.create(:definition => clean(params[:definition]))
    end
